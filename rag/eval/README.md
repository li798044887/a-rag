...
- `hotpot.py` … HotpotQA（dev distractor）から golden を生成。多ホップ検索＋answer 部分一致で `fact_coverage` を測る
...
HotpotQA（answer 部分一致で fact_coverage が埋まる多ホップ suite）を回す場合:

```bash
docker compose --profile worker up -d --build rag
docker compose exec -T rag uv run python -m eval prepare-hotpot --suite hotpot_dev
docker compose exec -T rag uv run python -m eval ingest \
