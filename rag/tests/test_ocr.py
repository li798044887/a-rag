...
import app.parsing.ocr as ocr_module
...
def test_ocr_images_skips_all_images_when_reader_initialization_fails(tmp_path, monkeypatch):
    img_dir = tmp_path / "images"
    img_dir.mkdir()
    (img_dir / "a.png").write_bytes(b"\x89PNG\r\n")
    blocks = [ParsedBlock(type="image", image_path="images/a.png", page=0)]

    def boom():
        raise RuntimeError("easyocr model unavailable")

    monkeypatch.setattr(ocr_module, "_build_ocr", boom)
...
def test_build_ocr_uses_cache_dir_from_env(tmp_path, monkeypatch):
    calls = []

    class FakeEasyOCR:
        @staticmethod
        def Reader(langs, **kwargs):
            calls.append((langs, kwargs))
            return "reader"

    model_dir = tmp_path / "easyocr-models"
    monkeypatch.setenv("EASYOCR_MODEL_DIR", str(model_dir))
    monkeypatch.setattr(ocr_module, "_reader", None)
    monkeypatch.setitem(sys.modules, "easyocr", SimpleNamespace(Reader=FakeEasyOCR.Reader))

    assert ocr_module._build_ocr() == "reader"
